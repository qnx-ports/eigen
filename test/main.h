...
typename internal::enable_if<internal::is_same<T1, T2>::value, bool>::type is_same_type(const T1&, const T2&) {
  return true;
}
...
#define EIGEN_TEST_SCALAR_TEST_OVERLOAD(TYPE)                                                            \
  inline bool test_isApprox(TYPE a, TYPE b) { return internal::isApprox(a, b, test_precision<TYPE>()); } \
...
typename NumTraits<T>::Real get_test_precision(
    const T&, typename internal::enable_if<internal::is_arithmetic<typename NumTraits<T>::Real>::value, T>::type* = 0) {
