...
template <typename T>
typename Eigen::internal::enable_if<(T::Flags & RowMajorBit) == RowMajorBit, typename T::RowXpr>::type innervec(
    T& A, Index i) {
  return A.row(i);
}
...
void sparse_block(const SparseMatrixType& ref) {
  ...
  // test innerVector()
  {
    ...
    for (Index j = 0; j < (std::min)(outer, inner); ++j) {
      ...
      if (j > 0) VERIFY(RealScalar(j) == numext::real(m3.innerVector(j).lastCoeff()));
    }
    m3.makeCompressed();
    for (Index j = 0; j < (std::min)(outer, inner); ++j) {
      VERIFY(j == numext::real(m3.innerVector(j).nonZeros()));
