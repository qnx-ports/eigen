...
#define SVD_FOR_MIN_NORM(M) JacobiSVD<M, decltype(ColPivHouseholderQRPreconditioner)>
#define SVD_TEST_TYPE JacobiSVD
...
void jacobisvd_all_options(const MatrixType& input = MatrixType())
{
  MatrixType m = input;
  svd_fill_random(m);
  //svd_option_checks<MatrixType>(m, NoQRPreconditioner);
  svd_option_checks<MatrixType>(m, ColPivHouseholderQRPreconditioner);
  svd_option_checks<MatrixType>(m, HouseholderQRPreconditioner);
  svd_option_checks_full_only<MatrixType>(m, FullPivHouseholderQRPreconditioner); // FullPiv only used when computing full unitaries
}

template<typename MatrixType>
void jacobisvd_verify_assert(const MatrixType& m = MatrixType())
{
  //svd_verify_assert<MatrixType>(m, NoQRPreconditioner);
  svd_verify_assert<MatrixType>(m, ColPivHouseholderQRPreconditioner);
  svd_verify_assert<MatrixType>(m, HouseholderQRPreconditioner);
  svd_verify_assert_full_only<MatrixType>(m, FullPivHouseholderQRPreconditioner);
}
...
void jacobisvd_verify_inputs(const MatrixType& m = MatrixType()) {
  ...
  VERIFY((int)DefaultSVD::QRPreconditioner == (int)internal::SVDOptions::ColPivHouseholderQRPreconditioner);
  VERIFY(!defaultSvd.computeU());
  VERIFY(!defaultSvd.computeV());

  // ColPivHouseholderQR is always default in presence of other options.
  VERIFY(( (int)JacobiSVD<MatrixType, decltype(ComputeThinU)>::QRPreconditioner == (int)internal::SVDOptions::ColPivHouseholderQRPreconditioner ));
  VERIFY(( (int)JacobiSVD<MatrixType, decltype(ComputeThinV)>::QRPreconditioner == (int)internal::SVDOptions::ColPivHouseholderQRPreconditioner ));
  VERIFY(( (int)JacobiSVD<MatrixType, decltype(ComputeThinU | ComputeThinV)>::QRPreconditioner == (int)internal::SVDOptions::ColPivHouseholderQRPreconditioner ));
  VERIFY(( (int)JacobiSVD<MatrixType, decltype(ComputeFullU | ComputeFullV)>::QRPreconditioner == (int)internal::SVDOptions::ColPivHouseholderQRPreconditioner ));
  VERIFY(( (int)JacobiSVD<MatrixType, decltype(ComputeThinU | ComputeFullV)>::QRPreconditioner == (int)internal::SVDOptions::ColPivHouseholderQRPreconditioner ));
  VERIFY(( (int)JacobiSVD<MatrixType, decltype(ComputeFullU | ComputeThinV)>::QRPreconditioner == (int)internal::SVDOptions::ColPivHouseholderQRPreconditioner ));
}
...
EIGEN_DECLARE_TEST(jacobisvd)
{
  ...
  for(int i = 0; i < g_repeat; i++) {
    ...
    CALL_SUBTEST_16(( svd_option_checks<MatrixXcd>(noQRTest, NoQRPreconditioner) ));

    CALL_SUBTEST_15(( svd_check_max_size_matrix<Matrix<float, Dynamic, Dynamic, ColMajor, 13, 15>>(r, c, ColPivHouseholderQRPreconditioner) ));
    CALL_SUBTEST_15(( svd_check_max_size_matrix<Matrix<float, Dynamic, Dynamic, ColMajor, 15, 13>>(r, c, HouseholderQRPreconditioner) ));
    CALL_SUBTEST_15(( svd_check_max_size_matrix<Matrix<float, Dynamic, Dynamic, RowMajor, 13, 15>>(r, c, ColPivHouseholderQRPreconditioner) ));
    CALL_SUBTEST_15(( svd_check_max_size_matrix<Matrix<float, Dynamic, Dynamic, RowMajor, 15, 13>>(r, c, HouseholderQRPreconditioner) ));
