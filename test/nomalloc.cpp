...
template<typename MatrixType> void nomalloc(const MatrixType& m)
{
  ...
  */
  ...
  m2.col(0).noalias() -= m1.adjoint().selfadjointView(Upper_t{}) * m1.col(0);
  m2.col(0).noalias() -= m1.selfadjointView(Upper_t{}) * m1.row(0).adjoint();
  m2.col(0).noalias() -= m1.adjoint().selfadjointView(Upper_t{}) * m1.row(0).adjoint();

  m2.row(0).noalias() = m1.row(0) * m1.selfadjointView(Upper_t{});
  m2.row(0).noalias() -= m1.row(0) * m1.adjoint().selfadjointView(Upper_t{});
  m2.row(0).noalias() -= m1.col(0).adjoint() * m1.selfadjointView(Upper_t{});
  m2.row(0).noalias() -= m1.col(0).adjoint() * m1.adjoint().selfadjointView(Upper_t{});
  VERIFY_IS_APPROX(m2,m2);
  
  m2.selfadjointView(Lower_t{}).rankUpdate(m1.col(0),-1);
  m2.selfadjointView(Upper_t{}).rankUpdate(m1.row(0),-1);
  m2.selfadjointView(Lower_t{}).rankUpdate(m1.col(0), m1.col(0)); // rank-2
