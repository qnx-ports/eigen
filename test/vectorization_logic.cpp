...
EIGEN_DECLARE_TEST(vectorization_logic) {
  ...
  if (internal::packet_traits<float>::Vectorizable) {
    ...
    VERIFY(test_redux(Matrix<float, 5, 2>(), EIGEN_UNALIGNED_VECTORIZE ? LinearVectorizedTraversal : DefaultTraversal,
                      CompleteUnrolling));
  }
  ...
  if (internal::packet_traits<double>::Vectorizable) {
    ...
    VERIFY(test_redux(Matrix<double, 7, 3>(), EIGEN_UNALIGNED_VECTORIZE ? LinearVectorizedTraversal : DefaultTraversal,
                      CompleteUnrolling));
  }
