...
void bdcsvd_all_options(const MatrixType& input = MatrixType())
{
  MatrixType m = input;
  svd_fill_random(m);
  svd_option_checks<MatrixType>(m, {});
}
...
EIGEN_DECLARE_TEST(bdcsvd)
{
  ...
  for(int i = 0; i < g_repeat; i++) {
    ...
    CALL_SUBTEST_17(( bdcsvd_all_options<Matrix<double, 17, 20> >() ));
    CALL_SUBTEST_5(( bdcsvd_all_options<Matrix<double, Dynamic, 30> >(Matrix<double, Dynamic, 30>(r, 30)) ));
    CALL_SUBTEST_6(( bdcsvd_all_options<Matrix<double, 20, Dynamic> >(Matrix<double, 20, Dynamic>(20, c)) ));
    ...
    CALL_SUBTEST_15(( bdcsvd_all_options<Matrix<double, 27, 20, RowMajor>>() ));

    CALL_SUBTEST_16(( svd_check_max_size_matrix<Matrix<float, Dynamic, Dynamic, ColMajor, 20, 35>>(r, c, ColPivHouseholderQRPreconditioner) ));
    CALL_SUBTEST_16(( svd_check_max_size_matrix<Matrix<float, Dynamic, Dynamic, ColMajor, 35, 20>>(r, c, HouseholderQRPreconditioner) ));
    CALL_SUBTEST_16(( svd_check_max_size_matrix<Matrix<float, Dynamic, Dynamic, RowMajor, 20, 35>>(r, c, ColPivHouseholderQRPreconditioner) ));
    CALL_SUBTEST_16(( svd_check_max_size_matrix<Matrix<float, Dynamic, Dynamic, RowMajor, 35, 20>>(r, c, HouseholderQRPreconditioner) ));
  }
