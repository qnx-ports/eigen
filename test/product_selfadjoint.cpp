...
template<typename MatrixType> void product_selfadjoint(const MatrixType& m)
{
  ...
  VERIFY_IS_APPROX(m2, (m1 + v1 * v2.adjoint()+ v2 * v1.adjoint()).triangularView(Lower_t{}).toDenseMatrix());

  m2 = m1.triangularView(Upper_t{});
  m2.selfadjointView(Upper_t{}).rankUpdate(-v1,s2*v2,s3);
  VERIFY_IS_APPROX(m2, (m1 + (s3*(-v1)*(s2*v2).adjoint()+numext::conj(s3)*(s2*v2)*(-v1).adjoint())).triangularView(Upper_t{}).toDenseMatrix());

  m2 = m1.triangularView(Upper_t{});
  m2.selfadjointView(Upper_t{}).rankUpdate(-s2*r1.adjoint(),r2.adjoint()*s3,s1);
  VERIFY_IS_APPROX(m2, (m1 + s1*(-s2*r1.adjoint())*(r2.adjoint()*s3).adjoint() + numext::conj(s1)*(r2.adjoint()*s3) * (-s2*r1.adjoint()).adjoint()).triangularView(Upper_t{}).toDenseMatrix());
  ...
  if (rows>1)
  {
    ...
    VERIFY_IS_APPROX(m2, m3.triangularView(Lower_t{}).toDenseMatrix());
  }
}
