...
class UpperBidiagonalization {
 public:
  ...
  typedef HouseholderSequence<const MatrixType, const typename internal::remove_all<
                                                    typename Diagonal<const MatrixType, 0>::ConjugateReturnType>::type>
...
void upperbidiagonalization_inplace_blocked(MatrixType& A, BidiagType& bidiagonal, Index maxBlockSize = 32,
  ...
  for (k = 0; k < size; k += blockSize) {
    ...
    {
      upperbidiagonalization_inplace_unblocked(B, &(bidiagonal.template diagonal<0>().coeffRef(k)),
