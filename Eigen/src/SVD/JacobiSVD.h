...
class qr_preconditioner_impl<MatrixType, FullPivHouseholderQRPreconditioner, PreconditionIfMoreColsThanRows, true> {
 public:
  ...
  typedef typename internal::make_proper_matrix_type<Scalar, ColsAtCompileTime, RowsAtCompileTime, Options,
...
struct traits<JacobiSVD<MatrixType_, QRPreconditioner> > : traits<MatrixType_> {
 ...
 */
template <typename MatrixType_, int QRPreconditioner>
class JacobiSVD : public SVDBase<JacobiSVD<MatrixType_, QRPreconditioner> > {
...
template <typename MatrixType, int QRPreconditioner>
void JacobiSVD<MatrixType, QRPreconditioner>::allocate(Eigen::Index rows, Eigen::Index cols,
    ...
    for (Index p = 1; p < m_diagSize; ++p) {
      for (Index q = 0; q < p; ++q) {
        ...
        if (abs(m_workMatrix.coeff(p, q)) > threshold || abs(m_workMatrix.coeff(q, p)) > threshold) {
          ...
          if (internal::svd_precondition_2x2_block_to_be_real<MatrixType, QRPreconditioner>::run(m_workMatrix, *this, p,
                                                                                                 q, maxDiagEntry)) {
