...
void BDCSVD<MatrixType, Options>::divide(Eigen::Index firstCol, Eigen::Index lastCol, Eigen::Index firstRowW, Eigen::Index firstColW, Eigen::Index shift)
{
  ...
  if (n < m_algoswap)
  {
    ...
    if (m_compV)
    {
      JacobiSVD<MatrixXr, decltype(ComputeFullU | ComputeFullV)> baseSvd;
      computeBaseCase(baseSvd, n, firstCol, firstRowW, firstColW, shift);
    } 
    else
    {
      JacobiSVD<MatrixXr, decltype(ComputeFullU)> baseSvd;
      computeBaseCase(baseSvd, n, firstCol, firstRowW, firstColW, shift);
    }
    return;
  }
...
BDCSVD<typename MatrixBase<Derived>::PlainObject, internal::DecompositionOptionsTag<Options>>
MatrixBase<Derived>::bdcSvd(internal::DecompositionOptionsTag<Options>) const
{
