...
class SparseLUTransposeView : public SparseSolverBase<SparseLUTransposeView<Conjugate, SparseLUType> > {
 ...
 public:
  ...
  SparseLUTransposeView(const SparseLUTransposeView& view) { this->m_sparseLU = view.m_sparseLU; }
    ...
    for (Index k = m_mapL.nsuper(); k >= 0; k--) {
      ...
      if (nsupc == 1) {
        ...
        Map<Matrix<Scalar, Dynamic, Dest::ColsAtCompileTime, ColMajor>, 0, OuterStride<> > U(
            &(X.coeffRef(fsupc, 0)), nsupc, nrhs, OuterStride<>(n));
  ...
  void solveTransposedInPlace(MatrixBase<Dest>& X) const {
    ...
    for (Index k = 0; k <= m_mapL.nsuper(); k++) {
      ...
      if (nsupc == 1) {
        ...
        Map<const Matrix<Scalar, Dynamic, Dynamic, ColMajor>, 0, OuterStride<> > A(&(m_mapL.valuePtr()[luptr]), nsupc,
                                                                                   nsupc, OuterStride<>(lda));
        Map<Matrix<Scalar, Dynamic, Dest::ColsAtCompileTime, ColMajor>, 0, OuterStride<> > U(&(X(fsupc, 0)), nsupc,
                                                                                             nrhs, OuterStride<>(n));
        if (Conjugate)
