...
template <typename T>
class Serializer<T, typename std::enable_if<std::is_trivial<T>::value && std::is_standard_layout<T>::value>::type> {
 public:
  /**
...
struct serialize_impl<N, T1, Ts...> {
  ...
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE uint8_t* serialize(uint8_t* dest, const T1& value, const Ts&... args) {
    Serializer serializer;
    dest = serializer.serialize(dest, value);
    return serialize_impl<N - 1, Ts...>::serialize(dest, args...);
  }

  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE uint8_t* deserialize(uint8_t* src, T1& value, Ts&... args) {
    Serializer serializer;
...
struct serialize_impl<0> {
  ...
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE uint8_t* serialize(uint8_t* dest) { return dest; }

  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE uint8_t* deserialize(uint8_t* src) { return src; }
};
 ...
 */
template <typename... Args>
EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE uint8_t* serialize(uint8_t* dest, const Args&... args) {
 ...
 */
template <typename... Args>
EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE uint8_t* deserialize(uint8_t* src, Args&... args) {
