...
::compute(const EigenBase<InputType>& a_matrix, int options)
{
  ...
  if(numext::is_zero_strict(scale)) scale = RealScalar(1);
...
ComputationInfo computeFromTridiagonal_impl(DiagType& diag, SubDiagType& subdiag, const Index maxIterations, bool computeEigenvectors, MatrixType& eivec)
{
  ...
  while (end>0)
  {
    ...
    while (end>0 && numext::is_zero_strict(subdiag[end-1]))
    {
      end--;
    }
    if (end<=0)
    ...
    start = end - 1;
    while (start>0 && !numext::is_zero_strict(subdiag[start-1]))
      start--;
...
static void tridiagonal_qr_step(RealScalar* diag, RealScalar* subdiag, Index start, Index end, Scalar* matrixQ, Index n)
{
  ...
  if(numext::is_zero_strict(td)) {
    mu -= numext::abs(e);
  } else if (!numext::is_zero_strict(e)) {
    const RealScalar e2 = numext::abs2(e);
    const RealScalar h = numext::hypot(td,e);
    if(numext::is_zero_strict(e2)) {
  ...
  for (Index k = start; k < end && !numext::is_zero_strict(z); ++k)
  {
