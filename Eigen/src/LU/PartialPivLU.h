...
struct partial_lu_impl {
  ...
  static Index unblocked_lu(MatrixTypeRef& lu, PivIndex* row_transpositions, PivIndex& nb_transpositions) {
    ...
    if (UnBlockedAtCompileTime) {
      ...
      if (Scoring()(lu(k, k)) == Score(0) && first_zero_pivot == -1) first_zero_pivot = k;
    }
...
void partial_lu_inplace(MatrixType& lu, TranspositionType& row_transpositions,
  ...
  partial_lu_impl<typename MatrixType::Scalar, MatrixType::Flags & RowMajorBit ? RowMajor : ColMajor,
                  ...
                  EIGEN_SIZE_MIN_PREFER_FIXED(MatrixType::RowsAtCompileTime,
                                              MatrixType::ColsAtCompileTime)>::blocked_lu(lu.rows(), lu.cols(),
                                                                                          &lu.coeffRef(0, 0),
                                                                                          lu.outerStride(),
                                                                                          &row_transpositions.coeffRef(
                                                                                              0),
                                                                                          nb_transpositions);
}
...
struct Assignment<DstXprType, Inverse<PartialPivLU<MatrixType> >,
