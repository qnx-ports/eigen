...
bool idrstabl(const MatrixType &mat, const Rhs &rhs, Dest &x,
  ...
  */
  ...
  if (S > 1) {
    ...
    // residual
    RealScalar FOM_residual =
        (h_FOM(S - 1, S - 2) * y(S - 2) * U.col(S - 1).head(N)).norm();
  ...
  while (k < maxIters) {
    for (Index j = 1; j <= L; ++j) {
      ...
      */
      ...
      for (Index i = 0; i < S; ++i) {
        sigma.col(i).noalias() =
            AR_T * precond.solve(U.block(N * (j - 1), i, N, 1));
      }
      ...
      for (Index q = 1; q <= S; ++q) {
        ...
        uvec.head(u.rows() * j) -=
            U.topRows(N * j) *
            lu_solver.solve(AR_T * precond.solve(u.col(j - 1)));
        ...
        V.block(0, q - 1, N * (j + 1), 1).noalias() =
            uvec.head(u.rows() * (j + 1));
      }
 ...
 *
 * This class allows to solve for A.x = b sparse linear problems. The vectors x
 * and b can be either dense or sparse. he Induced Dimension Reduction method.
 *
...
template <typename _MatrixType, typename _Preconditioner>
class IDRSTABL
    : public IterativeSolverBase<IDRSTABL<_MatrixType, _Preconditioner>> {
...
public:
  ...
  explicit IDRSTABL(const EigenBase<MatrixDerived> &A)
  ...
  */
  ...
  void _solve_vector_with_guess_impl(const Rhs &b, Dest &x) const {
    ...
    bool ret = internal::idrstabl(matrix(), b, x, Base::m_preconditioner,
                                  m_iterations, m_error, m_L, m_S);

    m_info = (!ret)
                 ? NumericalIssue
                 : m_error <= 10 * Base::m_tolerance ? Success : NoConvergence;
  }
