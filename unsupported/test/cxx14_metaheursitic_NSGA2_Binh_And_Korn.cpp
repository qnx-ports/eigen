...
void testNSGA2_Binh_and_Korn() {
  ...
  // the type of solver
  using solver_t =
      NSGA2<std::array<double, 2>, 2, FITNESS_LESS_BETTER, RecordOption::DONT_RECORD_FITNESS, args_t,
            Eigen::GADefaults<std::array<double, 2>, args_t, Std>::iFunNd,  // initializatoin functon
            nullptr,  // the fitness function can be assigned at runtime if nullptr is used
            Eigen::GADefaults<std::array<double, 2>, args_t, Std>::cFunNd<>,  // otherwise the function must be assigned
                                                                              // in the template
            Eigen::GADefaults<std::array<double, 2>, args_t, Std>::mFun_d>;   // This is suitable for iFun, fFun, cFun
                                                                              // and mFun.
